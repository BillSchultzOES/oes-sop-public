...
To deal with those issues, we recommend estimating treatment effects in factorial experiments using one of the following models, at least as a robustness check. As in the example above, assume two dichotomous Treatments, A and B (a subject can receive any combination of both treatments). We form the following factorial design groups: Not A or B (Group 1), A but not B (Group 2), B but not A (Group 3), and both A and B (Group 4). We can now define two options for analyzing these data using an OLS linear regression:

$$y = \alpha + \beta_{A}TreatA + \beta_{B}TreatB + \beta_{AB}(TreatA * TreatB)$$

$$y = \alpha + \beta_{G2}Group2 + \beta_{G3}Group3 + \beta_{G4}Group4$$

Which of these models is more appropriate, or more useful for communicating findings to an agency partner, will depend on study context.
