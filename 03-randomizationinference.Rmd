...
*Estimators*: Although we can never know the true causal effect of a new policy on our beneficiaries (see Chapter 1), we can provide a best guess. E.g.: "The average amount saved for retirement by people in the treatment group was \$1000 more than the average amount in the control group: our estimate of the average treatment effect is \$1000."
...
principle in practice [below](#randinfex).
...
Of course, we do not need to rely on simulation to define a standard error estimator. @gerber_field_2012 and @dunning_natural_2012 provide easy-to-read explanations for the following design based expression for the "feasible" (see below) standard error of a simple average treatment effect estimator. If we write $T$ as the set of all $m$ treated units and $C$ as the set of all $n-m$ non-treated units, we then have:
...
We'll compare the results of the simulation above to the standard error this expression produces. We'll also comapre both with the *true* standard error. We can calculate the true standard error of the ATE for this example data because we know the actual covariance between potential outcomes, having generated them ourselves. However, we can't observe this with real data. You can think of the expression above as a **feasible standard error**: a modified version that is estimable in real datasets and designed to be *at least as large* as the true standard error on average (i.e., it is intentionally conservative). Although we don't write it out, you can see an expression for the true standard error in our code.

First, let's calculate the true SE. We already have the simulated SE from the code above.
...
Lastly, we'll calculate another alternative called the *HC2 standard error*, which [@lin_agnostic_2013] shows to be a randomization or design based SE for OLS in particular (i.e., a design based alternative to the feasible SE that can be calcuated for treatment effects estimated using OLS). Like the feasible SE, HC2 errors should be conservative relative to the true SE (at least as large or larger). Below, we refer to this as the Neyman SE.

Note that under a sampling based inference perspective, this SE estimator can instead by justified as a method of relaxing the assumption of "identically" distributed errors across observations. In other words, it is robust to a problem called "heteroscedasticity," or "heteroscedasticity-consistent," hence the HC in its name.
...
These SE estimates in hand, let's review differences between the true standard error, the feasible standard error, the HC2 (Neyman) standard error, the standard error arising from direct repetition of the experiment, and the OLS standard error.

The Neyman SE (the HC2 OLS SE) is intended to be conservative relative to the true SE (at least as large or larger on average). We show this to be the case in our example. The Neyman SE is similar to the feasible SE outlined above, and both are larger than the true SE. We also illustrate the accuracy of our SE simulation as a way of thinking more intuitively about what the standard error represents. Lastly, recall that our design involves different outcome variances for the treated group and the control group. We would therefore expect what we are calling the "OLD iid" SE to be biased on average (different variances across treatment groups means that errors are not "identically" distributed). However, the fact that it is biased on average does not necessarily guarantee that it will be overly conservative or liberal in all cases.
