...
variable. We wish to assess the effect of some policy intervention on this continuous outcome. Our design calls for randomly assigning only $M = 75$ individuals to receive the intervention --- perhaps because it is very expensive to implement --- and the rest to control.

In addition to having data on the outcome and on treatment assignment, let's say that we also anticipate obtaining covariates for our 1,000 observations. This data contains two variables that are prognostic of the outcome *and the treatment effect*. We’ll call these `z1` and `z2`. The first is a continuous measure and the latter is a binary indicator. Our design calls for adjusting for these covariates to improve the precision of our estimated treatment effect. We can simulate such a design to illustrate the possible benefits of covariate adjustment in terms of improved statistical power.
...
<br />
```{r, highlight = F, eval = F}
## Generally, this approach to power calculation
## calls for estimating a distribution of t-statistics
## under the null (i.e. no y ~ x relationship in the data),
## but we'll incorporate this anyway to illustrate the impacts
## of treatment effect heterogeneity on the performance
## of these various estimators.
...
sim_given_delta <- function(delta) {
  fabricate(
    ...
    y = x*delta*z2 + x*delta + z1*0.3 + rnorm(N) # Generate y
    )
}

sim_across_deltas <- function(deltas) {
  lapply(deltas, function(.d) sim_given_delta(.d))
}
...
rep_data <-
  ...
  replicate_design(
    ...
    cz1 = z1 - mean(z1),     # Mean center z1
    cz2 = z2 - mean(z2), # Mean center z2
    x = complete_ra(N, m = N * 0.1), # Randomly assign 7.5% to treatment,
    y = x*0.5*z2 + x*0.5 + z1*0.3 + rnorm(N)
    )
```
:::
...
program define draw_from_design, nclass
	...
	* Simulate y
	gen y = z2*x*0.5 + z1*x*0.2 + rnormal()
...
rep_data <-
  ...
  replicate_design(
    ...
    cz1 = z1 - mean(z1),     # Mean center z1
    cz2 = z2 - mean(z2), # Mean center z2
    x = complete_ra(N, m = N * 0.25), # Randomly assign 7.5% to treatment
    cx = x - mean(x),
    y = 0.3*z1*cx - 0.5*z2*cx + rnorm(N) # Simulate Y
    )
```
