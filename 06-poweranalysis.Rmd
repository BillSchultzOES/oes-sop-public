...
Before we get to that template code, note that there are a few ways we could use a simulation to think about a design's statistical power:

- Simulate a design in which the null hypothesis is true many times, and estimate a treatment effect for each simulated dataset, yielding a distribution of test statistics (e.g., a t-static) under a true null. Use this null distribution, alongside estimates from each model, to calculate power for different possible effect sizes.
