...
simulation-based (i.e., computational) approach. We have one example below, and several more throughout this chapter. Our simulation examples relies on manually-written functions that could be adapted by OES team members to the needs of different projects. This code is more detailed than we need for simple power analysis problems, but it provides useful flexibility when simulating more complicated designs.

### How do we structure the simulation?

Before we get to that template code, though, there are a few ways we could use a simulation to think about a design's statistical power:
