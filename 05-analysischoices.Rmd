...
When we have background or baseline information about experimental units, we can use this to increase the precision with which we estimate treatment effects (i.e., increase the statistical power of our tests). We often try to use this information during the design phase to create block randomized designs. But this is not always feasible, so we also often pre-specify covariance adjustment as a part of our analysis to increase statistical power.

Researchers typically include covariance adjustment in a regression analysis by adding the covariates as controls in a linear, additive fashion (see $X_{i}$ equation 3 below). This estimator of the average treatment effect is consistent (in the sense of bias decreasing as $n \rightarrow \infty$, i.e. "asymptotically"), but it can be appreciably biased in smaller samples.^[The unadjusted difference in means is unbiased, but as @lin_agnostic_2013 points out, this is not reassuring if, for instance, treatment and control groups appear noticeably imbalanced despite randomization.] It can also be---counterintuitively---*less* efficient than an unadjusted difference in means [@freedman2008rae]. In contrast, an approach to covariate adjustment that we call the "Lin estimator" or "Lin adjustment" performs better in terms of asymptotic efficiency, and at worst will be no less efficient than an unadjusted difference in means [@lin_agnostic_2013; @negi2021revisiting]. Though it is also only consistent and not unbiased, the Lin estimator generally performs better in terms of RMSE (root mean squared error), a metric that incorporates both bias and precision [@negi2021revisiting].

The precision-loss attributable to linear covariance adjustment estimator may often be quite small in practice when sample sizes are large. Yet, because it is frequently costless to use the Lin estimator in large samples, Lin adjustment is our default recommendation (see [this page](https://declaredesign.org/blog/2018-09-11-controlling-pretreatment-covariates.html) as well). That said, it is not uncommon for projects to encounter situations where linear covariate adjustment is still preferred, either for practical reasons (e.g., simplifying regression models that are already complex for other reasons) or statistical reasons (situations where the benefits of Lin estimation are negligible; see below).
