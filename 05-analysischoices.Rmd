...
```{r RItest1, eval=FALSE, cache=F,include=F, echo=F}
...
## Define custom estimator that recalculates variance weights
varweightest <- function(data) {
  temp <- data
  icc <- ICCest(x=clusterF,y=Y,data=temp)
  temp$varweight <- 1/(iccres$vara + (iccres$varw/temp$nb))
  mod <- lm_robust(Y~Zcluster+nb, weights=varweight, clusters=cluster, se_type="CR2", data=temp)
  data.frame(
    estimate = mod$coefficients["Zcluster"],
    p.value = mod$p.value["Zcluster"],
    conf.low = mod$conf.low["Zcluster"],
    conf.high = mod$conf.high["Zcluster"]
    )
}

## Linear regression, CR2 errors, control for cluster size, variance weights
estC7 <- declare_estimator(handler = label_estimator(varweightest), label="C7: OLS with Cluster Control and Var. Weights, CR2 RCSE")
