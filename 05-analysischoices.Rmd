...
What we call Lin adjustment produces equivalent treatment effect estimates to a different procedure that has been called things like the "oaxaca-blinder estimator" [@guo2023generalized], "full regression adjustment" [@negi2021revisiting], or "G-computation" [@snowden2011implementation; @greifer2023estimating]. In a simple two-arm randomized trial, it proceeds as follows:

1. Create separate subsets of treated and control units

2. Regress the outcome on controls separately within each subset

3. Using the control subset model, calculated a predicted control potential outcome for treated units

4. Using the treated subset model, calculated a predicted treatment potential outcome for control units

5. Calculate the unit-level difference between the real and predicted potential outcomes
...
dat1$g_y0[dat1$Z == 1] <- predict(mod_c, newdata = dat1[dat1$Z == 1,])
dat1$g_y1[dat1$Z == 0] <- predict(mod_t, newdata = dat1[dat1$Z == 0,])
dat1$g_y0[dat1$Z == 0] <- dat1$Y[dat1$Z == 0]
dat1$g_y1[dat1$Z == 1] <- dat1$Y[dat1$Z == 1]
