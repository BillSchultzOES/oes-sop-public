...
like times, counts of events, dollars, percentages, etc.) we generally assess the **weak null hypothesis** of no average effects and estimate an average treatment effect. We may also assess the **sharp null hypothesis** of no effect for any unit using some test statistic beside a difference-in-means. What we call the weak null corresponds to the null evaluated by default in many software packages (like R and Stata) when implementing procedures like a difference-in-means test or a linear regression. Meanwhile, the sharp null is more often evaluated when employing design-based permutation inference procedures (example below). Note that, particularly in smaller samples, tests of the sharp null may be somewhat better powered [@reichardt1999justifying]. It may also be possible to justify a hypothesis test of the sharp null in situations where justifying other procedures is more difficult [@rubin1986comment].^[As @rubin1986comment discusses, the stable unit treatment value assumption (SUTVA) holds trivially when testing the sharp null via permutation based inference. But this is not the case when estimating average treatment effects or testing the weak null. This is important for thinking through the difficult problem of evaluating impacts of "attributes" rather than manipulable treatments.]
...
The standard errors we prefer, HC2, reflect repeated randomization from a fixed experimental pool. As discussed in Chapter 3, @lin_agnostic_2013 and @samii_equivalencies_2012 show that the standard error estimator of an unbiased average treatment effect within a "finite-sample" or design based framework is equivalent to what is called the HC2 standard error. In R, these SEs are produced by default by the `estimatr` package's function `difference_in_means()` and the `lmtest` package's functions `coeftest()` and `coefci()`. They can also be produced using the `vcovHC()` function from the `sandwich` package. In Stata, when using `regress`, these can be estimated using the `vce(hc2)` option (they are not the default used by the `robust` option). Some other commands offer a similar option; check the help file to see.
...
It is also relatively common for OES evaluations to encounter situations where these canned approaches aren't useful (e.g., the study design employs an uncommon randomization scheme). It is useful to be able to program randomization inference manually as well. We provide templates for how this could be done in the code below, focusing on complete random assignment. This yields similar results to the canned functions above.
...
To see this equivalence, consider that difference-in-proportion standard errors are estimated with the following equation:
