...
Notice that simply adjusting $p$-values from this linear model ignores the fact that we may be interested in other pairwise comparisons, such as the difference in effects between receiving `T3` vs `T4`. And as already emphasized, we could be leaving meaningful statistical power "on the table" by applying procedures that ignore correlations between our test statistics.
...
In contrast, in Example 2, the interventions are justified separately, and the truth of $H_{0, int1}$ presumably has no bearing on the truth of $H_{0, int2}$. We have determined that it is not useful to only know that at least one of the two modifications decreased application processing times (i.e., rejecting the intersection of their nulls is not informative)---we need to know whether each modification on its own was effective. Therefore, we would not adjust for multiple testing. The collective false positive rate is still inflated, but we are not drawing a collective conclusion across tests, and the Type I error rates for the separate tests remain uninflated [@rubin2024inconsistent]. Returning to the three simulated p-values from the multiple arms adjustment example code, we can see that the estimated false positive rate under a true null is approximately correct when each test is considered alone.
...
When we have additional information about experimental units (covariates), we can use this to increase the the statistical power of our tests. When possible, we use this information during the design phase to randomize interventions within blocks. But we also often pre-specify covariance adjustment as a part of our analyses. The basic idea is that using covariates to help explain variation in $Y$ unrelated to a treatment effect helps us estimate that treatment effect more precisely.

Researchers typically do this by regressing the outcome on a treatment indicator and a set of linear, additive terms for each covariate (like $X_{i}$ in equation 3 below). This estimator of the average treatment effect can be biased, though that bias usually reduces quickly as sample size increases (see below). The unadjusted difference in means would in theory remain unbiased in these cases, though as @lin_agnostic_2013 points out, this is not reassuring if we think adjusting for covariates is necessary for a specific evaluation (e.g., to account for meaningful imbalances that persist despite randomization). More importantly, standard linear covariance adjustment can sometimes be counter-intuitively *less efficient than an unadjusted difference in means* [@freedman2008rae].^[This is more likely as the probability of treatment gets further from 0.5, or if the relationship between covariates and treatment effect heterogeneity is stronger than the relationship between covariates and the potential outcomes. See @lin_agnostic_2013 p. 304, remark *v*.]

An approach to covariance adjustment that we prefer called the **Lin estimator** or **Lin adjustment** performs better---at worst, it will be no less asymptotically efficient than an unadjusted difference in means [@lin_agnostic_2013]. We explain how it is implemented and provide more intuition for its relative efficiency below. Like linear covariance adjustment, the Lin estimator can also be biased in small samples. But note that it will be unbiased if it correctly models the true conditional expectation of $Y$ given treatment and the covariates [@negi2021revisiting; @lin_agnostic_2013]. For instance, this condition is met when the model is "saturated" with controls for every combination of covariate and treatment values [@angrist2009mostly], such as when using the Lin estimator to adjust for a set of dummies representing block fixed effects [@miratrix2013adjusting]. In contrast, under linear adjustment for block fixed effects, we would *by definition* not be "saturating" our model with the terms that estimate separate block fixed effects in each treatment arm.

Though Lin adjustment has better theoretical properties, precision loss due to standard covariance adjustment may often be small in practice, especially relative to the loss of precision that we would need for our statistical conclusions to change. Yet, it is frequently costless to use the Lin estimator in large enough samples, so it is still our default recommendation (see [this page](https://declaredesign.org/blog/2018-09-11-controlling-pretreatment-covariates.html) as well). But our evaluations do sometimes encounter situations where we prefer to stick with standard linear covariance adjustment, either on practical or statistical grounds. A few examples:
...
We expand on some of these issues in the subsections below, with more detail on how Lin estimation works. We also provide a simulated example illustrating hypothetical power differences between standard covariate adjustment and Lin adjustment in Chapter 6.
