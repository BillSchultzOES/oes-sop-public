...
In contrast, an approach to covariance adjustment that we call the **Lin estimator** or **Lin adjustment** performs better---at worst, it will be no less asymptotically efficient than an unadjusted difference in means [@lin_agnostic_2013]. We explain how it is implemented below. Like linear covariance adjustment, the Lin estimator is also generally only consistent and not unbiased. But it will be unbiased if it correctly models the true conditional expectation of $Y$ given treatment and the covariates [@negi2021revisiting; @lin_agnostic_2013].^[As will linear covariate adjustment, though this condition is relatively less likely in the lienar adjustment case; see the next footnote.] This condition is met when the model is "saturated" with controls for every combination of covariate values, such as when using the Lin estimator to account for a set of dummies representing block fixed effects [@miratrix2013adjusting].^[Notice that this is not possible under linear covariate adjustment, since by assumption under linear adjustment we are not "saturating" our model with the terms that estimate separate block fixed effects in each treatment arm.]
