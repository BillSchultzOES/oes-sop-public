...
These issues in mind, our default procedures for evaluating multi-arm trials are as follows:
 ...
 - Second, perform the rest of the comparisons as **exploratory** analyses without multiple testing adjustment --- i.e., as analyses that may inform future projects and give hints about where we might be seeing more or less of an effect, but which cannot serve as a foundation for policy conclusions on their own.
 
 - Alternatively, we might instead make multiple confirmatory comparisons and adjust for their collective false positive rate.^[We generally seek to control the FWER, but sometimes consider controlling the false discovery rate, or FDR, instead. Holding the FDR at 20%, for example, implies trying to ensure that only 20% of reported test results (or 1 in 5) are a false positive. This contrasts with adjustments to control the FWER, which focus on the probability of observing one or more false positives across tests.]
 
Other options we consider in some cases are using the Tukey HSD procedure for pairwise comparisons (illustrated below), or testing the hypotheses in a particular order to preserve statistical power [@rosenbaum2008a].
...
Our standard practice when making multiple confirmatory comparisons is to adjust the FWER for uncorrelated tests using Holm adjustment. For more on such adjustments and multiple comparisons see EGAP's [10 Things you need to know about multiple comparisons](https://egap.org/resource/10-things-to-know-about-multiple-comparisons/).
...
Though, simply adjusting $p$-values from this linear model ignores the fact that we may be interested in other pairwise comparisons, such as the difference in effects between receiving `T3` vs `T4`. It also ignores potential correlations in the distribution of test statistics (i.e., we are leaving statistical power "on the table").

Instead of the above, and instead of employing simulation to control the FWER (step 7 on [this page](https://egap.org/resource/10-things-to-know-about-multiple-comparisons/)), we can implement a [Tukey Honestly Signficant Differences (HSD) test](https://en.wikipedia.org/wiki/Tukey%27s_range_test). The Tukey HSD test (sometimes called a Tukey range test or just a
...
When we have additional information about experimental units (covariates), we can use this to increase the the statistical power of our tests. When we can, we use this information during the design phase to randomize interventions within blocks. But we also often pre-specify covariance adjustment as a part of our confirmatory analyses.

Researchers typically do this by regressing the outcome on a treatment indicator and a set of linear, additive terms for each covariate (like $X_{i}$ in equation 3 below). This estimator of the average treatment effect is *consistent* (bias decreasing as $n \rightarrow \infty$, i.e. "asymptotically"), but *is technically not unbiased* in finite samples. The risk of appreciable bias is greater in especially small samples (see below).^[The unadjusted difference in means, in constrast, is unbiased, but as @lin_agnostic_2013 points out, this is not reassuring if, for instance, treatment and control groups appear noticeably imbalanced despite randomization.] It can also counterproductively *less efficient* than an unadjusted difference in means in some cases [@freedman2008rae].^[This becomes more likely as the probability of treatment gets further from 0.5, or if the relationship between covariates and treatment effect heterogeneity is stronger than the relationship between covariates and the potential outcomes. See @lin_agnostic_2013 p. 304, remark *v*.] In contrast, an approach to covariance adjustment that we call the **Lin estimator** or **Lin adjustment** *performs better in terms of asymptotic efficiency*---at worst, it will be no less asymptotically efficient than an unadjusted difference in means [@lin_agnostic_2013; @negi2021revisiting]. Though it is also only consistent and not unbiased, the Lin estimator generally performs better in terms of RMSE (root mean squared error), a metric that incorporates both bias and precision [@negi2021revisiting].^[The Lin estimator will be unbiased if it accurately models the conditional expectation of $Y$ given treatment and the covariates, though this is unlikely in practice. See @negi2021revisiting, 510-511.]

The precision-loss attributable to linear covariance adjustment estimator can be quite small when sample sizes are large. Yet, because it is frequently costless to use the Lin estimator in large samples, Lin adjustment is our default recommendation (see [this page](https://declaredesign.org/blog/2018-09-11-controlling-pretreatment-covariates.html) as well). That said, it is not uncommon for projects to encounter situations where linear covariate adjustment is still preferred, either for practical grounds (e.g., simplifying regression models that are already complex for other reasons) or statistical grounds (situations where the benefits of Lin estimation are likely negligible):

- The randomization design has only two arms, and the probability of receiving treatment is approximately 0.5

- We do not expect the covariates to be associated with meaningful treatment effect heterogeneity

- The number of parameters in a model is large enough relative to the sample size that the loss of degrees of freedom from using Lin adjustment (i.e., the finite sample efficiency costs of adding more terms to the model) is substantial; Lin adjustment is only more efficient *asymptotically*, as sample size increases indefinitely

We expand on some of these issues in the subsections below.
...
assigned and is thus independent of background variables like $X$. However in any given finite sized experiment $\cov(X,Z) \ne 0$, so this does not reduce to an unbiased estimator as it does in the bivariate case. Thus, @freedman2008rae showed that there is a risk of bias in using the equation above to estimate the average treatment effect.

As discussed above, this kind of covariance adjustment is generally also less asymptotically efficient. To resolve the efficiency issue, @lin_agnostic_2013 suggests the following least squares approach --- regressing the outcome on binary treatment assignment $Z_i$ and its interaction with mean-centered covariates:^[Lin adjustment could also mitigate bias in some circumstances, if for instance the only covariates are dummies for mutually exclusive subgroups or block fixed effects [@lin_agnostic_2013; @miratrix2013adjusting].]
...
Here, we show how typical covariance adjustment can lead to bias or inefficiency in estimation of the average treatment effect, increaseing the overall RMSE (root mean squared error)---and how using the Lin procedure (or increasing sample size) can reduce the RMSE. In this case, we compare an experiment with 20 units to an experiement with 100 units, in each case with half of the units assigned to treatment by complete random assignment.
